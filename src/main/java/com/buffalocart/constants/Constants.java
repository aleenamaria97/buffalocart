package com.buffalocart.constants;

public class Constants {
    public static final String CONFIG_FILE = "\\src\\main\\resources\\config.properties";
    public static final String EXCEL_FILE_PATH="\\src\\main\\resources\\TestData.xlsx";
    public static final String EXCEL_SHEET_LOGIN_PAGE="Login";
    public static final String EXCEL_SHEET_HOME_PAGE="Home";
    public static final String EXCEL_SHEET_FORGOT_PASSWORD_PAGE="ForgotPassWord";
    public static final String EXCEL_SHEET_USER_MANAGEMENT_PAGE="UserManagement";
    public static final String EXCEL_SHEET_USER_PAGE="User";
    public static final String EXCEL_SHEET_ADD_USER_PAGE="AddUser";
}
